...
<body class="bg-gray-200 min-h-screen leading-none">
    ...
    @if(session('status'))
        <div class="bg-teal-500 p-8 text-center text-white font-bold uppercase">{{ session('status') }}</div>
    @endif
                ...
                <div class="flex items-center justify-around">
                    ...
                    <nav class="flex-1 text-right">
                        ...
                        @else
                            ...
                            <a href="{{ route('notifications') }}" class="bg-teal-500 rounded-full mr-2 px-3 py-1 font-bold text-sm text-white">{{ Auth::user()->unreadNotifications->count() }}</a>
