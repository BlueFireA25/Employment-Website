...
@section('content')
    <h1 class="text-2xl text-center mt-10">Edit Vacant {{$vacant->title}}</h1>

    <form class="max-w-lg mx-auto my-10" action="{{ route('vacancies.update', ['vacant' => $vacant->id]) }}" method="POST">
        ...
        <div class="mb-5">
            ...
            <select
                ...
                @foreach($categories as $category)
                    <option
                        {{ $vacant->category_id == $category->id ? 'selected' : ''}}
                        value="{{ $category->id }}">
                        {{ $category->name }}
                    </option>
                @endforeach
        ...
        <div class="mb-5">
            ...
            <select
                ...
                @foreach($experiences as $experience)
                    <option
                        {{ $vacant->experience_id == $experience->id ? 'selected' : ''}}
                        value="{{ $experience->id }}">
                        {{ $experience->name }}
                    </option>
                @endforeach
        ...
        <div class="mb-5">
            ...
            <select
                ...
                @foreach($locations as $location)
                    <option
                        {{ $vacant->location_id == $location->id ? 'selected' : ''}}
                        value="{{ $location->id }}">
                        {{ $location->name }}
                    </option>
                @endforeach
        ...
        <div class="mb-5">
            ...
            <select
                ...
                @foreach($salaries as $salary)
                    <option
                        {{ $vacant->salary_id == $salary->id ? 'selected' : '' }}
                        value="{{ $salary->id }}">
                        {{ $salary->name }}
                    </option>
                @endforeach
        ...
        <div class="mb-5">
            ...
            <skill-list
                ...
                :oldskills="{{ json_encode($vacant->skills) }}">
            </skill-list>
